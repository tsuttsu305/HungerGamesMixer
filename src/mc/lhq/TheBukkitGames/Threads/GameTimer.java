package mc.lhq.TheBukkitGames.Threads;
...
public class GameTimer implements Runnable{
	...
	public void run() {
		...
		if(time==LoadFiles.WaitEndTime){
			...
			if(time==LastBattleStartTime){
				...
				TheBukkitGames.GamingWorld.strikeLightning(TheBukkitGames.GamingWorld.getSpawnLocation().add(0.0D, 100.0D, 0.0D));
			...
			if(time==EndGamingTime){
				TheBukkitGames.server.shutdown();
			}
		}else{
	...
	public static void SetPlayersType(PlayerType pt,boolean statesreset,boolean pointreset){
		...
		while(u!=pl.length){
			...
			GamePlayerListener.pltype.put(p.getName(), TheBukkitGames.serverstate);
			}else{
			}
			u++;
		}
